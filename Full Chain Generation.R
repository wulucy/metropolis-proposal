...
symmetric_beta_22 <- function(curr_state) {
  ...
  t_ji <- dbeta((curr_state-prop_state+1)/2, 2, 2)
...
asymmetric_beta_31 <- function(curr_state) {
  ...
  t_ji <- dbeta((curr_state-prop_state+1)/2, 3, 1)
...
full_mh <- function(steps, prop_function, mean, sd) {
  ...
  for(i in 1:steps) {
    ...
    t_ij_list <- c(t_ij_list, t_ij)
    t_ji_list <- c(t_ji_list, t_ji)
    pi_j_list <- c(pi_j_list, dnorm(prop_state, mean, sd))
    pi_i_list <- c(pi_i_list, dnorm(curr_state, mean, sd))
  ...
  result <- data.frame(curr_state_list, prop_state_list, t_ij_list, t_ji_list, pi_j_list, pi_i_list)
